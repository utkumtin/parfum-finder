...
merge. It is then driven through `search_site` with `fetch()` swapped for a
router that serves that site's own `fixtures/<id>/` capture instead of the
network. Nothing served is invented: the search page is a trimmed excerpt of the
real search.html, cut out with the profile's own selectors, containing only the
...
from parfum_finder import engine as engine_module
from parfum_finder.engine import search_site
from parfum_finder.extract import select_field
from parfum_finder.fetch import FetchResult
...
def _install_fake_fetch(
    ...
    *,
    ...
    monkeypatch.setattr(engine_module, "fetch", fake_fetch)
...
async def test_every_target_site_is_driven_from_its_real_profile(
    monkeypatch: pytest.MonkeyPatch, site_id: str
) -> None:
    profile = load_site_profile(_SITES_DIR / f"{site_id}.json")
    _install_fake_fetch(monkeypatch, site_id, profile)
...
async def test_venco_reads_its_three_real_decant_prices(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    _install_fake_fetch(monkeypatch, "venco", profile)
...
async def test_decantall_reads_its_four_real_decant_prices(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    _install_fake_fetch(monkeypatch, "decantall", profile)
...
async def test_decantall_search_page_uses_the_playwright_override(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    _install_fake_fetch(monkeypatch, "decantall", profile, calls=calls)
...
async def test_luxurydekant_reads_its_real_decant_prices_and_drops_the_30ml_row(
    ...
    _install_fake_fetch(monkeypatch, "luxurydekant", profile)
...
async def test_ruxangroup_drops_the_full_bottle_next_to_the_decants(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    _install_fake_fetch(monkeypatch, "ruxangroup", profile)
...
async def test_dekantparfum_drives_the_post_endpoint_for_a_real_price(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    _install_fake_fetch(monkeypatch, "dekantparfum", profile)
...
async def test_dekantdoktoru_drives_the_post_endpoint_for_a_real_price(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    _install_fake_fetch(monkeypatch, "dekantdoktoru", profile)
