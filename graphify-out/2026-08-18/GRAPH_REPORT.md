...
## Corpus Check
- 125 files · ~309,604 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2494 nodes · 7029 edges · 100 communities (95 shown, 5 thin omitted)
...
Cohesion: 0.07
Nodes (60): CandidateFilter, HTMLParser, _candidates_to_open(), _check_variant_control(), _fetch_page(), _headers(), _is_excluded(), _jitter_s() (+52 more)
...
Cohesion: 0.09
Nodes (48): CacheKey, Path, Run one site and classify what came back instead of raising.      It is also whe, Run every site against one query, all at once, and report each separately., run_site(), run_sites(), FetchResult, One fetched page, uniform regardless of which strategy produced it. (+40 more)
...
Cohesion: 0.17
Nodes (37): Lock, Any, A site's display name, with a badge when its profile is old enough     to be wor, Show what storage already knows, then go to the shops for the rest.      `force=, run_scan(), site_label(), now_iso(), Return the current UTC time as 'YYYY-MM-DDTHH:MM:SSZ'.      Every timestamp writ (+29 more)
...
Cohesion: 0.15
Nodes (42): BaseModel, AcceptedSearch, _add_basket_item(), _AppState, BasketAddRequest, BasketQtyRequest, The FastAPI app: a thin HTTP/WS wrapper around the Faz 1 services.  No business, _read_basket() (+34 more)
...
Cohesion: 0.09
Nodes (30): api, ApiError, readDetail(), request(), Window, App(), Toast, View (+22 more)
...
Cohesion: 0.07
Nodes (29): _Change, format_ml(), format_price(), Format a price for display (comma-thousands, dot-decimal).      Decimal('1250'), Format a volume for display (dot-decimal): Decimal('1.5') -> '1.5 ml'., BasketScreen, _heading(), _leg_block() (+21 more)
...
Cohesion: 0.09
Nodes (56): Pressed, _paced_fetcher(), Protocol, What one site had to say about one query, and how much to trust it.      Four st, One site's pacing state, for as long as whoever holds it says.      The gate and, Wrap a fetcher so one site's requests go out one at a time, spaced apart.      T, What a caller needs of run_site, as a type callers can stand a fake in for., SitePace (+48 more)
...
- **Why does `SearchScreen` connect `TUI App Shell` to `.__init__`, `_ResultRow`, `TUI App & Screens`, `Search/Basket Domain Models`, `TUI Confirm Dialog`, `ConfirmDialog.tsx`, `run_sites`, `Search TUI Screen`, `Price/Size Normalization`, `_FixtureFetcher`, `Static`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Why does `FetchResult` connect `Offline Profile Validation` to `validate_live`, `_retry_after_s`, `Fetch Strategy Probing`, `HTTP/Browser Fetching`, `Search/Basket Domain Models`, `Search Engine Core`, `_named_profile`, `Basket Store & Pricing`, `_css_variant`, `exclude_keywords`, `Product Extraction`, `run_sites`, `Basket Site Scenarios`, `ResultRow`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `SiteRunner` connect `run_sites` to `.__init__`, `Title Matcher`, `Search/Basket Domain Models`, `TUI Confirm Dialog`, `TUI App Shell`, `Search Engine Core`, `FieldConfidence`, `_ResultRow`, `Offline Profile Validation`, `Candidate Filtering`, `_FixtureFetcher`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
