...
## Summary
- 2474 nodes · 6749 edges · 104 communities (95 shown, 9 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 380 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6e1a6eeb`
...
1. `PerfumeQuery` - 69 edges
2. `search_site()` - 66 edges
3. `SearchScreen` - 64 edges
4. `_profile()` - 58 edges
5. `discover()` - 56 edges
6. `match_title()` - 51 edges
7. `_write_profile()` - 50 edges
8. `SiteResult` - 48 edges
9. `connect()` - 48 edges
...
Cohesion: 0.05
Nodes (60): BasketRefreshEvent, BasketRow, CachedPrice, Lock, Pressed, ResultRow, ScanEvent, SnapshotRow (+52 more)
...
Cohesion: 0.11
Nodes (29): cached_prices(), _perfume_id(), _product_id(), Connection, datetime, SQLite persistence: an append-only price history.  Tables: sites, perfumes, prod, Write one scan's reading of one size, and return its snapshot id.      The perfu, Write a whole scan at once and return how many prices were recorded.      Every (+21 more)
...
Cohesion: 0.06
Nodes (63): HTMLParser, _check_empty_search(), _check_variant_control(), ExtractionFailed, _fetch_page(), _headers(), _is_excluded(), _paced_fetcher() (+55 more)
...
Cohesion: 0.09
Nodes (29): ProductCandidate, One hit on a search results page, before its product page is opened.      `raw_t, One decant size of one product, in the units the database stores.      Tenths of, A candidate together with the decant sizes its product page offers., What one site had to say about one query, and how much to trust it.      Four st, SearchHit, SiteResult, Variant (+21 more)
...
Cohesion: 0.24
Nodes (28): MonkeyPatch, SiteResult, _basket_row(), _collect(), _ok_result(), _profile(), Any, Path (+20 more)
...
Cohesion: 0.11
Nodes (25): BasketReport, compare_split_to_best_full(), Every site's single-site scenario, split by whether it covers everything.      A, The cheapest basket split the search found. A heuristic, not a proof.      Every, Score a split plan against the cheapest full-coverage single site.      Only the, SplitPlan, _heading(), _leg_block() (+17 more)
...
Cohesion: 0.09
Nodes (40): parse_query(), product_label(), Split one typed line, "Dior Sauvage EDP", into the three identity parts.      Th, Reduce a site's own title to the product it is about, spelled one way.      What, Whether a search result's own listing text is worth opening the page for.      J, title_could_match(), Tests for parfum_finder.matcher.  The thing being defended here is not "does it, test_a_brand_the_shop_writes_out_in_full_does_not_cost_the_match() (+32 more)
...
Cohesion: 0.11
Nodes (29): connect(), Path, Open the price database, creating the schema if it isn't there yet.      Foreign, _counting_runner(), _days_ago(), Any, What the widget actually paints, with markup applied.      `.content` is the raw, Wait for the scan to end, which is the only time the table is filled.      Count (+21 more)
...
Cohesion: 0.07
Nodes (45): _age_line(), _age_of(), Check, _count_result_cards(), _first_result_url(), _LayerUnavailable, live_query(), _no_results_check() (+37 more)
...
Cohesion: 0.16
Nodes (12): Collection, build_basket_rows(), _label(), Basket scenario evaluation. A pure function, no network access, no sqlite.  Inpu, One site's share of a split basket: what to buy there and what it costs.      `s, Name one basket line the way a missing-item warning has to read it., Turn basket lines and their site prices into what the table shows.      Out of s, What it would cost to buy some or all of the basket from one site.      `covered (+4 more)
...
### Community 80 - "_collect_products"
Cohesion: 0.50
...
- **Why does `SearchScreen` connect `TUI App Shell` to `TUI App & Screens`, `Title Matcher`, `ResultRow`, `TUI Confirm Dialog`, `._scan`, `_css_variant`, `ConfirmDialog.tsx`, `exclude_keywords`, `Static`, `Search TUI Screen`, `FetchResult`, `_FixtureFetcher`?**
  _High betweenness centrality (0.050) - this node is a cross-community bridge._
- **Why does `PerfumeQuery` connect `FetchResult` to `_ResultRow`, `Title Matcher`, `Search Engine per Site`, `TUI Confirm Dialog`, `TUI App Shell`, `Basket Store & Pricing`, `SplitPlan`, `exclude_keywords`, `Playwright Errors`, `Search TUI Screen`, `Candidate Filtering`, `Live Profile Validation`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Why does `BasketScreen` connect `_FixtureFetcher` to `validate_live`, `Site Profiles & Templates`, `Title Matcher`, `TUI App & Screens`, `TUI Confirm Dialog`, `TUI App Shell`, `Basket Optimizer Core`, `Static`, `Price/Size Normalization`?**
