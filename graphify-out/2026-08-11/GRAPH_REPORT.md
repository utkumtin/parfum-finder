...
## Corpus Check
- 70 files · ~259,054 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1836 nodes · 4689 edges · 96 communities (70 shown, 26 thin omitted)
...
Cohesion: 0.14
Nodes (9): _Change, BasketReport, Every site's single-site scenario, split by whether it covers everything.      A, BasketScreen, Path, The basket: the list on top, one scenario per site underneath., The three inputs basket.py's pure functions score: items, prices, shipping., _remove() (+1 more)
...
Cohesion: 0.09
Nodes (46): Node, SiteHooks, _check_empty_search(), _check_variant_control(), _fetch_page(), _headers(), _is_excluded(), _paced_fetcher() (+38 more)
...
Cohesion: 0.11
Nodes (36): CacheKey, Path, Run one site and classify what came back instead of raising.      It is also whe, Run every site against one query, all at once, and report each separately., run_site(), run_sites(), _counting_fetcher(), _profile() (+28 more)
...
Cohesion: 0.08
Nodes (41): Exception, LogCaptureFixture, Match, find_header_columns(), find_match(), open_worksheet(), Any, Writing a search result onto the user's own Google Sheets wishlist.  The wishlis (+33 more)

### Community 20 - "Snapshot Writing"
Cohesion: 0.12
Nodes (31): apply_variant_rules(), One decant size of one product, in the units the database stores.      Tenths of, Turn raw size rows into decant variants, dropping what is not a decant.      Thr, Convert a price in lira to whole kuruş.      Integers all the way, never a float, _to_kurus(), Variant, _named_profile(), Any (+23 more)

### Community 21 - "Candidate Filtering"
Cohesion: 0.23
Nodes (14): Run one site's profile against the real site.      Same contract as offline mode, validate_live(), _DeadSite, _FakeSite, _fixture_site(), M5's own criterion: when a profile stops agreeing with its site's real markup, o, A stand-in for one live site, answering the search page then the rest.      Live, A host that cannot be reached at all. (+6 more)

### Community 22 - "Basket Site Scenarios"
Cohesion: 0.24
Nodes (10): live_query(), _path(), A URL's path with no trailing slash, for comparing two spellings of one page., Every site that has a profile, sorted so reports read the same way twice., The query this site's fixture was captured with, read back out of its URL., Validate every site against the live web, or just the ones named.      Serial li, site_ids(), validate_all_live() (+2 more)

### Community 23 - "Price/Size Normalization"
Cohesion: 0.15
Nodes (21): casefold_tr(), _classify_single_separator(), format_ml(), _parse_number(), parse_price(), parse_size_ml(), Decimal, Number parsing and formatting for prices and volumes, plus text folding.  This i (+13 more)
...
Cohesion: 0.13
Nodes (25): HTMLParser, Fetcher, Protocol, Anything that can stand in for `fetch`.      Offline profile validation runs the, Check, _count_result_cards(), _first_result_url(), _LayerUnavailable (+17 more)
...
Cohesion: 0.08
Nodes (32): RuntimeError, _candidates_to_open(), ExtractionFailed, CandidateFilter, A page answered but gave up nothing, where something was expected.      This is, A candidate together with the decant sizes its product page offers., Run one query against one site and read every hit's sizes.      Everything site-, Narrow the search results down to the pages worth a request.      The first one (+24 more)

### Community 48 - "._build_rows"
Cohesion: 0.10
Nodes (23): _balanced_value(), _embedded_documents(), extract_css_variants(), extract_endpoint_variants(), _flatten_jsonld(), _loads_or_skip(), Any, Turn one Product and everything under it into rows. (+15 more)

### Community 50 - "Profile Age Checks"
Cohesion: 0.28
Nodes (15): Check one site's profile against that site's saved fixtures.      Never raises f, validate_offline(), _corrupted_sites_dir(), Any, Path, A sites/ directory holding one real profile with fields overwritten.      The re, test_a_dead_price_selector_is_caught_as_the_extraction_step(), test_a_dead_search_selector_is_caught_as_the_search_step() (+7 more)

### Community 51 - "._refresh_table"
Cohesion: 0.09
Nodes (11): Decimal, HeaderSelected, Row, RowSelected, One priced size, exactly as the table shows it and as a keypress needs it., The initial screen: search bar, streaming results table, notices, footer., What each site charges for the product a block is about.          One entry per, The default order: typed order, product, site, size.          The typed order co (+3 more)
...
Cohesion: 0.12
Nodes (30): basket_lines(), basket_prices(), basket_sites(), BasketLine, BasketPrice, BasketSite, _perfume_id(), _product_id() (+22 more)
...
Cohesion: 0.50
Nodes (4): now_iso(), Return the current UTC time as 'YYYY-MM-DDTHH:MM:SSZ'.      Every timestamp writ, test_now_iso_matches_the_required_format(), test_now_iso_string_order_matches_chronological_order()

### Community 57 - "_FixtureFetcher"
Cohesion: 0.11
Nodes (14): Changed, Fetcher, SnapshotRow, _listing_filter(), Any, CandidateFilter, PerfumeQuery, SiteResult (+6 more)

### Community 58 - "ComposeResult"
Cohesion: 0.13
Nodes (20): One site's share of a split basket: what to buy there and what it costs.      `s, The cheapest basket split the search found. A heuristic, not a proof.      Every, SplitLeg, SplitPlan, format_price(), Format a price for display (comma-thousands, dot-decimal).      Decimal('1250'), _heading(), _leg_block() (+12 more)

### Community 59 - "_named_profile"
Cohesion: 0.22
Nodes (7): _ClimbState, Basket scenario evaluation. A pure function, no network access, no sqlite.  Inpu, The hill-climb's working assignment plus the running per-site figures.      `sub, One site's shipping terms, read once and reused for every scenario.      `free_s, What it would cost to buy some or all of the basket from one site.      `covered, ShippingConfig, SiteScenario
...
Cohesion: 0.16
Nodes (9): What one site had to say about one query, and how much to trust it.      Four st, SiteResult, _BasketRow, format_age(), Any, Turn a price age in days into the words the age column shows., One basket line as the table shows it, with the keys a keypress needs., The age column exists to be glanced at, so it is phrased, not printed. (+1 more)
...
Cohesion: 0.29
Nodes (7): _age_of(), profile_age_days(), datetime, Whole days between a profile's `discovered_at` and now.      Only the exact UTC, The profile's age in days, or None if the file cannot say.      Reads the site f, test_a_timestamp_that_is_not_the_schema_format_is_rejected(), test_profile_age_is_counted_in_whole_days_from_the_schema_timestamp()
...
Cohesion: 0.50
Nodes (4): timeout_s, default, minimum, type
...
- **Why does `BasketScreen` connect `Title Matcher` to `TUI App & Screens`, `_ResultRow`, `TUI Confirm Dialog`, `ComposeResult`, `Basket Optimizer Core`, `conftest.py`, `ComposeResult`, `_named_profile`, `format_age`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `search_site()` connect `Basket Domain Logic` to `Site Profiles & Templates`, `basket_prices`, `Search Engine Core`, `Decant Variant Rules`, `Offline Profile Validation`, `Profile Age Checks`, `Snapshot Writing`, `Candidate Filtering`?**
