# Graph Report - parfum-finder  (2026-08-23)

## Corpus Check
- 133 files · ~313,490 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2538 nodes · 7213 edges · 100 communities (92 shown, 8 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 514 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7b54177f`
...
- `_RecordingClient` --uses--> `UpdateDownload`  [INFERRED]
  tests/test_api.py → src/parfum_finder/updater.py
- `_RecordingStream` --uses--> `UpdateDownload`  [INFERRED]
  tests/test_api.py → src/parfum_finder/updater.py
...
Cohesion: 0.13
Nodes (32): browser_session(), fetch(), Strategy, Fetch one URL using exactly the given strategy.      `method`/`data` exist for t, Yield a fetcher that keeps one browser for every playwright page it reads., test_the_no_results_page_would_otherwise_read_as_suspect(), _fake_launch(), Event (+24 more)
...
Cohesion: 0.08
Nodes (52): HTMLParser, _check_variant_control(), _fetch_page(), _headers(), _jitter_s(), _page_evidence(), _page_offers_sizes(), _page_says_sold_out() (+44 more)
...
Cohesion: 0.05
Nodes (64): Client, Exception, MonkeyPatch, check_enabled(), check_for_update(), DownloadProgress, fetch_latest_release(), handoff_command() (+56 more)
...
Cohesion: 0.09
Nodes (37): Command-line entry point.  Subcommands will be added incrementally as the projec, _attempt(), _count_jsonld(), _count_product_objects(), _detect_platforms(), format_report(), _label_platform(), _one_line() (+29 more)
...
Cohesion: 0.13
Nodes (5): HeaderSelected, The initial screen: search bar, streaming results table, notices, footer., Show what storage already knows, then go to the shops for the rest.          `fo, Empty the table for a new scan.          The columns are the same every time now, SearchScreen
...
Cohesion: 0.22
Nodes (3): Changed, Close out a submit that named no perfume anyone could look for., Submitted
...
Cohesion: 0.06
Nodes (56): _choose_strategy(), collect_prices(), DiscoveryReport, FieldConfidence, _flatten_defaults(), _format_choice(), _format_confidence(), _format_defaults() (+48 more)

### Community 49 - "_RecordingFetcher"
Cohesion: 0.09
Nodes (40): conn(), Connection, Path, Tests for parfum_finder.store: the timestamp helper and the schema.  The one har, Two lines added within the same second must still read back the same way twice., A disabled site loses its basket column, but an enabled quiet one keeps one., NULL means the site has no free shipping tier at all, not a threshold of zero., The table's CHECK (qty > 0) would reject a bare 0, and the '-' key has to     su (+32 more)
...
Cohesion: 0.19
Nodes (21): apply_variant_rules(), _is_excluded(), Decimal, Turn raw size rows into decant variants, dropping what is not a decant.      Thr, Read one row's volume in millilitres, or None if the text does not say.      "fi, Whether this row is something other than a decant.      The size threshold is in, Convert a price in lira to whole kuruş.      Integers all the way, never a float, _read_size_ml() (+13 more)
...
Cohesion: 0.07
Nodes (37): _Change, BasketReport, compare_split_to_best_full(), Every site's single-site scenario, split by whether it covers everything.      A, The cheapest basket split the search found. A heuristic, not a proof.      Every, Score a split plan against the cheapest full-coverage single site.      Only the, SplitPlan, format_ml() (+29 more)
...
Cohesion: 0.09
Nodes (35): _as_str(), _build_offer(), _build_product(), _coerce_in_stock(), _collect_offers(), _collect_products(), _collect_variants(), _css_variant() (+27 more)
...
Cohesion: 0.07
Nodes (61): Pressed, ScanEvent, BasketRefreshEvent and viewmodel dataclasses as JSON-safe dicts.  Eve, _paced_fetcher(), Protocol, What one site had to say about one query, and how much to trust it.      Four st, One site's pacing state, for as long as whoever holds it says.      The gate and, Wrap a fetcher so one site's requests go out one at a time, spaced apart.      T, What a caller needs of run_site, as a type callers can stand a fake in for. (+53 more)
...
### Community 90 - "extract_endpoint_variants"
Cohesion: 0.50
...
- **Why does `SearchScreen` connect `Basket Site Scenarios` to `validate_live`, `_ResultRow`, `TUI App & Screens`, `Search/Basket Domain Models`, `TUI Confirm Dialog`, `Static`, `run_sites`, `Search TUI Screen`, `FetchResult`, `Candidate Filtering`, `._write_to_sheet`, `_FixtureFetcher`, `_jitter_s`, `SQLite Store`, `field_map`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `PerfumeQuery` connect `_ResultRow` to `TUI Confirm Dialog`, `run_sites`, `Playwright Errors`, `Search TUI Screen`, `FetchResult`, `Candidate Filtering`, `Basket Site Scenarios`, `._write_to_sheet`, `cli.py`, `._apply_scan_event`, `Live Profile Validation`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `FetchResult` connect `Title Matcher` to `Fetch Strategy Probing`, `HTTP/Browser Fetching`, `Search/Basket Domain Models`, `Search Engine Core`, `_css_variant`, `validate_live`, `Product Extraction`, `Offline Profile Validation`, `run_sites`, `_wait_until`, `Price/Size Normalization`, `apply_variant_rules`, `_named_profile`, `Discovery CLI Reporting`?**
