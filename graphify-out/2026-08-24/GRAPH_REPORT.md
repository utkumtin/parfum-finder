...
## Corpus Check
- 134 files · ~317,061 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2599 nodes · 7436 edges · 97 communities (93 shown, 4 thin omitted)
...
Cohesion: 0.06
Nodes (123): Screen, One decant size of one product, in the units the database stores.      Tenths of, A candidate together with the decant sizes its product page offers., SearchHit, Variant, Write one scan's reading of one size, and return its snapshot id.      The perfu, record_snapshot(), ParfumFinderApp (+115 more)
...
Cohesion: 0.08
Nodes (61): CaptureFixture, ask_which_platform(), main(), Any, Connection, Path, Command-line entry point.  Subcommands will be added incrementally as the projec, Scan every site for the perfumes named, store what came back, print it.      One (+53 more)
...
Cohesion: 0.07
Nodes (69): HTMLParser, _canonical_path(), canonical_url(), _check_variant_control(), _decode_unreserved(), _fetch_page(), _has_product_ancestor(), _headers() (+61 more)
...
Cohesion: 0.08
Nodes (54): Run one query against one site and read every hit's sizes.      Everything site-, Try spelling variants until this site returns an emittable match.      The match, run_site_attempts(), search_site(), _named_profile(), Any, Path, Tests for the profile-driven search in parfum_finder.engine.  What these defend (+46 more)
...
Cohesion: 0.29
Nodes (11): format_report(), Validate every site, or just the ones named.      Serial rather than concurrent:, Render the validations as the offline half of the report in APP_FLOW §6.      A, validate_all_offline(), _iso_days_ago(), A discovered_at stamp that lands a fixed number of days in the past.      Relati, test_a_fresh_profile_gets_no_age_line(), test_a_future_dated_profile_is_called_out_rather_than_read_as_fresh() (+3 more)
...
Cohesion: 0.10
Nodes (16): BasketResponse, BasketRow, BestCombination, SiteScenario, basket(), basketRow(), resultRow(), scenario() (+8 more)
...
Cohesion: 0.11
Nodes (30): basket_prices(), cached_prices(), _perfume_id(), _product_id(), Connection, datetime, SQLite persistence: an append-only price history.  Tables: sites, perfumes, prod, Write a whole scan at once and return how many prices were recorded.      Every (+22 more)
...
Cohesion: 0.08
Nodes (74): BaseModel, Collection, AcceptedSearch, _add_basket_item(), _AppState, BasketAddRequest, BasketQtyRequest, _load_profiles() (+66 more)
...
Cohesion: 0.11
Nodes (25): authToken(), readDetail(), request(), Window, UpdateDialog(), AcceptedSearch, BasketReport, RefreshStart (+17 more)

### Community 45 - "_ResultRow"
Cohesion: 0.11
Nodes (58): TestClient, _auth(), _client(), db_path(), _ok_result(), Any, MonkeyPatch, Path (+50 more)
...
Cohesion: 0.19
Nodes (17): refusalReason(), streamUrl(), useEventStream(), ProgressBar(), basketKey(), formatAge(), formatMl(), formatPerMl() (+9 more)
...
Cohesion: 0.29
Nodes (14): Check one site's profile against that site's saved fixtures.      Never raises f, validate_offline(), _corrupted_sites_dir(), Any, Path, A sites/ directory holding one real profile with fields overwritten.      The re, test_a_dead_price_selector_is_caught_as_the_extraction_step(), test_a_dead_search_selector_is_caught_as_the_search_step() (+6 more)
...
Cohesion: 0.15
Nodes (41): Lock, _record_search(), Any, A site's display name, with a badge when its profile is old enough     to be wor, Show what storage already knows, then go to the shops for the rest.      `force=, run_scan(), site_label(), connect() (+33 more)
...
Cohesion: 0.17
Nodes (15): CacheKey, CandidateFilter, _candidates_to_open(), Path, Run one matcher-aware spelling sequence for every site in parallel., Narrow the search results down to the pages worth a request.      The first one, Run every site against one query, all at once, and report each separately., run_sites() (+7 more)
...
Cohesion: 0.13
Nodes (17): AddButton(), Badge(), BadgeKind, ConfirmDialog(), ScanStatus(), VerdictAddButton(), wishlistIdentity, wishlistKey() (+9 more)
...
Cohesion: 0.07
Nodes (57): Pressed, Protocol, What one site had to say about one query, and how much to trust it.      Four st, One site's pacing state, for as long as whoever holds it says.      The gate and, What a caller needs of run_site, as a type callers can stand a fake in for., SitePace, SiteResult, SiteRunner (+49 more)
...
Cohesion: 0.16
Nodes (12): api, ApiError, App(), Toast, View, root, daysSince(), SearchScreen() (+4 more)
...
Cohesion: 0.50
Nodes (4): Check, _no_results_check(), Why an empty results page is suspicious, or why it is not.      A full page that, One agreement between a profile and its site, and whether it still holds.      `
...
Cohesion: 0.12
Nodes (28): FastAPI, create_app(), HTTP/WS backend for the GUI frontend. See api/app.py for the app itself., encode_basket_refresh_event(), encode_basket_report(), encode_basket_row(), encode_result_row(), encode_scan_event() (+20 more)
...
- **Why does `PerfumeQuery` connect `_ResultRow` to `TUI Confirm Dialog`, `CLI Entry Points`, `Fixture Fetcher (Tests)`, `run_sites`, `Offline Profile Validation`, `Search TUI Screen`, `FetchResult`, `Basket Site Scenarios`, `apply_variant_rules`, `._apply_scan_event`, `Live Profile Validation`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `SiteRunner` connect `run_sites` to `_named_profile`, `Title Matcher`, `Search/Basket Domain Models`, `Search Engine Core`, `Fixture Fetcher (Tests)`, `_ResultRow`, `Offline Profile Validation`, `Playwright Errors`, `FetchResult`, `Basket Site Scenarios`, `_FixtureFetcher`, `_scenario_block`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `BasketScreen` connect `_FixtureFetcher` to `TUI App & Screens`, `Site Profiles & Templates`, `Static`, `Fixture Fetcher (Tests)`, `Basket Optimizer Core`, `run_sites`, `Basket Site Scenarios`, `_scenario_block`?**
