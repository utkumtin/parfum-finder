...
Cohesion: 0.10
Nodes (51): Collection, BasketRow, _score_basket(), basket_inputs(), BasketItem, optimize(), BasketRow, Prices (+43 more)
...
Cohesion: 0.12
Nodes (55): BaseModel, FastAPI, AcceptedSearch, _add_basket_item(), _AppState, BasketAddRequest, BasketQtyRequest, create_app() (+47 more)
...
Cohesion: 0.13
Nodes (22): CacheKey, CandidateFilter, _candidates_to_open(), Path, Narrow the search results down to the pages worth a request.      The first one, Try spelling variants until this site returns an emittable match.      The match, Run every site against one query, all at once, and report each separately., Run one matcher-aware spelling sequence for every site in parallel. (+14 more)
...
Cohesion: 0.07
Nodes (58): Pressed, ScanEvent, BasketRefreshEvent and viewmodel dataclasses as JSON-safe dicts.  Eve, _paced_fetcher(), One site's pacing state, for as long as whoever holds it says.      The gate and, Wrap a fetcher so one site's requests go out one at a time, spaced apart.      T, SitePace, Fetcher, Protocol (+50 more)
...
Cohesion: 0.10
Nodes (6): HeaderSelected, RowSelected, Any, Path, The initial screen: search bar, streaming results table, notices, footer., SearchScreen

### Community 99 - "exclude_keywords"
Cohesion: 0.29
...
- **Why does `SiteRunner` connect `Fixture Fetcher (Tests)` to `_named_profile`, `.on_mount`, `TUI App Shell`, `Search Engine Core`, `_css_variant`, `_ResultRow`, `run_sites`, `Playwright Errors`, `_RecordingFetcher`, `FetchResult`, `_scenario_block`, `_seed_site`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `PlaywrightNotInstalled` connect `Playwright Errors` to `Title Matcher`, `HTTP/Browser Fetching`, `Search Engine Core`, `Fixture Fetcher (Tests)`, `FieldConfidence`, `._build_rows`, `run_sites`, `_RecordingFetcher`, `_wait_until`, `BasketRow`, `_named_profile`, `._apply_scan_event`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `PerfumeQuery` connect `_ResultRow` to `_named_profile`, `.on_mount`, `_trial`, `Fixture Fetcher (Tests)`, `run_sites`, `Node`, `Search TUI Screen`, `FetchResult`, `._apply_scan_event`, `_seed_site`, `Live Profile Validation`?**
