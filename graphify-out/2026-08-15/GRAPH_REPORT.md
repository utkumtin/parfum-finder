...
## Corpus Check
- 83 files · ~270,118 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2045 nodes · 5833 edges · 77 communities (72 shown, 5 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 374 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f1898297`
...
Cohesion: 0.08
Nodes (51): HTMLParser, _check_variant_control(), _fetch_page(), _headers(), _is_excluded(), _paced_fetcher(), _page_offers_sizes(), _page_says_sold_out() (+43 more)
...
Cohesion: 0.14
Nodes (24): BasketReport, compare_split_to_best_full(), Every site's single-site scenario, split by whether it covers everything.      A, The cheapest basket split the search found. A heuristic, not a proof.      Every, Score a split plan against the cheapest full-coverage single site.      Only the, SplitPlan, format_price(), Format a price for display (comma-thousands, dot-decimal).      Decimal('1250') (+16 more)
...
Cohesion: 0.10
Nodes (28): _canonical(), _covers(), _ends_with(), _index_of(), _match_text(), _own_identity(), Perfume matching: brand and concentration are mandatory; fuzzy matching only app, Split one typed line into the perfumes it asks for, on " - ".      The separator (+20 more)
...
Cohesion: 0.08
Nodes (63): Match, match_title(), parse_query(), PerfumeQuery, product_label(), The perfume being looked for, split into its three identity parts.      `concent, One site title judged against the query.      `concentration` is what the title, Split one typed line, "Dior Sauvage EDP", into the three identity parts.      Th (+55 more)
...
### Community 37 - "_trial"
Cohesion: 0.09
Nodes (39): Collection, basket_inputs(), build_basket_rows(), _ClimbState, _label(), Basket scenario evaluation. A pure function, no network access, no sqlite.  Inpu, One site's share of a split basket: what to buy there and what it costs.      `s, The hill-climb's working assignment plus the running per-site figures.      `sub (+31 more)

### Community 38 - "TUI Confirm Dialog"
Cohesion: 0.09
Nodes (55): Pressed, Protocol, What one site had to say about one query, and how much to trust it.      Four st, What a caller needs of run_site, as a type callers can stand a fake in for., SiteResult, SiteRunner, Fetcher, Protocol (+47 more)
...
Cohesion: 0.09
Nodes (40): conn(), Connection, Path, Tests for parfum_finder.store: the timestamp helper and the schema.  The one har, A disabled site loses its basket column, but an enabled quiet one keeps one., NULL means the site has no free shipping tier at all, not a threshold of zero., Deleting a row that's already gone is a race between two screens, not a bug., An update aimed at a row that isn't there means the caller is out of sync. (+32 more)
...
Cohesion: 0.20
Nodes (35): SiteResult, TestClient, _auth(), _client(), db_path(), _ok_result(), Any, Path (+27 more)
...
Cohesion: 0.20
Nodes (29): Lock, Any, ScanEvent, A site's display name, with a badge when its profile is old enough     to be wor, Show what storage already knows, then go to the shops for the rest.      `force=, run_scan(), site_label(), now_iso() (+21 more)
...
Cohesion: 0.07
Nodes (12): Changed, HeaderSelected, RowSelected, Row, ScanEvent, The initial screen: search bar, streaming results table, notices, footer., Close out a submit that named no perfume anyone could look for., Show what storage already knows, then go to the shops for the rest.          `fo (+4 more)

### Community 52 - "FetchResult"
Cohesion: 0.08
Nodes (48): BaseModel, BasketLine, BasketPrice, BasketRefreshEvent, BasketReport, BasketSite, FastAPI, ResultRow (+40 more)

### Community 53 - "conftest.py"
Cohesion: 0.15
Nodes (24): add_basket_item(), basket_lines(), _perfume_id(), _product_id(), Connection, datetime, SQLite persistence: an append-only price history.  Tables: sites, perfumes, prod, Write one scan's reading of one size, and return its snapshot id.      The perfu (+16 more)
...
Cohesion: 0.11
Nodes (21): Any, Connection, Command-line entry point.  Subcommands will be added incrementally as the projec, Scan every perfume against every site and print each site as it lands.      One, Write one site's rows in a transaction of its own.      Per site rather than per, One line per site: which site, how it went, and what it said.      The site id i, _report_line(), _scan_all() (+13 more)
...
### Community 73 - "_LayerUnavailable"
Cohesion: 0.67
...
- **Why does `PerfumeQuery` connect `_ResultRow` to `_trial`, `TUI Confirm Dialog`, `JsonLdProduct`, `Search TUI Screen`, `._refresh_table`, `conftest.py`, `snapshot_rows`, `Live Profile Validation`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `extract_jsonld_products()` connect `JSON-LD Product Extraction` to `Search Engine Core`, `Product Extraction`, `._build_rows`, `Candidate Filtering`, `_FixtureFetcher`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `discover()` connect `Platform Discovery Flow` to `Site Profiles & Templates`, `CLI Entry Points`, `format_age`, `._build_rows`, `Playwright Errors`, `snapshot_rows`?**
