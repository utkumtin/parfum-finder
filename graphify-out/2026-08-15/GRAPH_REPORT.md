...
## Corpus Check
- 83 files · ~269,730 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2026 nodes · 5939 edges · 74 communities (70 shown, 4 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 422 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6b6ed45e`
...
Cohesion: 0.07
Nodes (24): _Change, BasketReport, Every site's single-site scenario, split by whether it covers everything.      A, format_age(), format_price(), Format a price for display (comma-thousands, dot-decimal).      Decimal('1250'), Turn a price age in days into the words the age column shows., BasketScreen (+16 more)
...
Cohesion: 0.07
Nodes (121): Screen, ProductCandidate, One hit on a search results page, before its product page is opened.      `raw_t, One decant size of one product, in the units the database stores.      Tenths of, A candidate together with the decant sizes its product page offers., SearchHit, Variant, ParfumFinderApp (+113 more)
...
Cohesion: 0.29
Nodes (16): default_db_path(), default_log_path(), ensure_user_data(), fixtures_dir(), hooks_dir(), is_frozen(), platforms_dir(), Path (+8 more)
...
Cohesion: 0.09
Nodes (47): _read_basket(), basket_inputs(), build_basket_rows(), _ClimbState, compare_split_to_best_full(), _label(), Basket scenario evaluation. A pure function, no network access, no sqlite.  Inpu, One site's share of a split basket: what to buy there and what it costs.      `s (+39 more)
...
### Community 37 - "_trial"
Cohesion: 0.12
Nodes (39): BaseModel, FastAPI, _AppState, BasketAddRequest, BasketQtyRequest, create_app(), The FastAPI app: a thin HTTP/WS wrapper around the Faz 1 services.  No business, RefreshStartResponse (+31 more)

### Community 38 - "TUI Confirm Dialog"
Cohesion: 0.08
Nodes (64): Lock, Pressed, Protocol, What one site had to say about one query, and how much to trust it.      Four st, What a caller needs of run_site, as a type callers can stand a fake in for., SiteResult, SiteRunner, Fetcher (+56 more)
...
Cohesion: 0.09
Nodes (38): Connection, Tests for parfum_finder.store: the timestamp helper and the schema.  The one har, A stale reading must never outrank the one taken after it.      latest_prices al, A disabled site loses its basket column, but an enabled quiet one keeps one., NULL means the site has no free shipping tier at all, not a threshold of zero., Deleting a row that's already gone is a race between two screens, not a bug., The table's CHECK (qty > 0) would reject a bare 0, and the '-' key has to     su, A snapshot pointing at a variant that doesn't exist has to be rejected.      SQL (+30 more)

### Community 44 - "Decant Variant Rules"
Cohesion: 0.21
Nodes (13): live_query(), _path(), A URL's path with no trailing slash, for comparing two spellings of one page., Every site that has a profile, sorted so reports read the same way twice., Validate every site, or just the ones named.      Serial rather than concurrent:, The query this site's fixture was captured with, read back out of its URL., Validate every site against the live web, or just the ones named.      Serial li, site_ids() (+5 more)

### Community 45 - "_ResultRow"
Cohesion: 0.24
Nodes (29): TestClient, _auth(), _client(), db_path(), _ok_result(), Any, Path, Tests for parfum_finder.api: FastAPI TestClient, a fake SiteRunner, a temp sqlit (+21 more)
...
Cohesion: 0.33
Nodes (20): _basket_row(), _collect(), _ok_result(), _profile(), Any, MonkeyPatch, Path, Tests for parfum_finder.services.scan: the event stream a fake SiteRunner drives (+12 more)
...
Cohesion: 0.09
Nodes (29): _age_of(), Check, _count_result_cards(), _first_result_url(), _LayerUnavailable, _no_results_check(), _probe_layer(), _probe_other_layers() (+21 more)
...
Cohesion: 0.06
Nodes (15): Changed, HeaderSelected, RowSelected, ComposeResult, Any, ComposeResult, Path, Row (+7 more)

### Community 52 - "FetchResult"
Cohesion: 0.11
Nodes (21): _add_basket_item(), _load_profiles(), Any, Path, _remove_basket_item(), _set_basket_qty(), _site_summary(), _sync_profiles() (+13 more)

### Community 53 - "conftest.py"
Cohesion: 0.13
Nodes (28): add_basket_item(), basket_prices(), cached_prices(), now_iso(), _perfume_id(), _product_id(), Connection, datetime (+20 more)
...
Cohesion: 0.19
Nodes (20): grouped_value(), Decimal, Pure sorting and grouping rules for the results table.  No I/O, no Textual state, What each site charges for the product a block is about.      One entry per site, The default order: typed order, product, site, size.      The typed order comes, The order once a column has been picked: the site layer drops out.      Asking f, site_ranks(), sorted_value() (+12 more)
...
- **Why does `SearchScreen` connect `._refresh_table` to `TUI App & Screens`, `Title Matcher`, `Search/Basket Domain Models`, `_trial`, `TUI Confirm Dialog`, `Search TUI Screen`, `FetchResult`, `Discovery CLI Reporting`?**
  _High betweenness centrality (0.042) - this node is a cross-community bridge._
- **Why does `PerfumeQuery` connect `Discovery CLI Reporting` to `_ResultRow`, `Title Matcher`, `_trial`, `TUI Confirm Dialog`, `JsonLdProduct`, `Search TUI Screen`, `Snapshot Writing`, `conftest.py`, `._refresh_table`, `snapshot_rows`, `Live Profile Validation`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `PlaywrightNotInstalled` connect `HTTP/Browser Fetching` to `Fetch Strategy Probing`, `Search/Basket Domain Models`, `Search Engine per Site`, `TUI Confirm Dialog`, `Search Engine Core`, `FieldConfidence`, `._build_rows`, `Offline Profile Validation`, `Playwright Errors`, `Basket Site Scenarios`, `Request Schema Fields`, `setup_logging`?**
