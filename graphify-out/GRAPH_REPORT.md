...
## Summary
- 2431 nodes · 6632 edges · 99 communities (90 shown, 9 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 366 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d9fcddfc`
...
7. `SiteResult` - 48 edges
8. `match_title()` - 48 edges
9. `connect()` - 48 edges
...
- `test_a_write_failure_still_reports_the_rows_and_counts_as_an_error()` --indirect_call--> `RowsReady`  [INFERRED]
  tests/test_scan_service.py → src/parfum_finder/services/scan.py
- `test_run_basket_refresh_excludes_a_suspect_site_with_a_notice()` --indirect_call--> `BasketRefreshStarted`  [INFERRED]
  tests/test_scan_service.py → src/parfum_finder/services/scan.py
...
Cohesion: 0.13
Nodes (10): Pressed, CacheHit, A request already reported via `SiteFinished` could not be written to     storag, Once, before any request goes out.      `total_perfumes` counts only what storag, One perfume was answered from storage, not from the shops.      `age_days` is th, ScanStarted, WriteFailed, ConfirmScreen (+2 more)
...
Cohesion: 0.17
Nodes (44): _app(), _painted_in_basket(), _per_query_runner(), Path, Tests for the search screen: grouping, persistence, and the key bindings.  A fak, The table stays empty for the whole scan, and the bar carries the news.      Row, Answer each perfume with a row of its own, and record every scan asked for., A brand-only search sent while a scan runs must not freeze the screen.      The (+36 more)

### Community 5 - "Search Engine per Site"
Cohesion: 0.13
Nodes (26): _load_profiles(), Any, Path, _recent_searches(), _record_search(), _remove_basket_item(), _set_basket_qty(), _site_summary() (+18 more)
...
Cohesion: 0.06
Nodes (61): CacheKey, CandidateFilter, HTMLParser, _candidates_to_open(), _check_variant_control(), _fetch_page(), _headers(), _is_excluded() (+53 more)
...
Cohesion: 0.08
Nodes (36): The Textual App root. Handles screen navigation and is the app's default entry p, _age_of(), _count_result_cards(), _first_result_url(), _LayerUnavailable, live_query(), _path(), _probe_layer() (+28 more)
...
Cohesion: 0.14
Nodes (40): Lock, MonkeyPatch, ScanEvent, SiteResult, BasketPriceExcluded, Any, SiteRunner, One (site, perfume) request is done, whatever it ended in.      `status` is the (+32 more)
...
Cohesion: 0.07
Nodes (30): BasketRefreshEvent, BasketRow, CachedPrice, _Change, ResultRow, SnapshotRow, _about(), BasketRefreshFinished (+22 more)
...
Cohesion: 0.14
Nodes (10): DEFAULT_CONFIG, EMPTY_BASKET, FakeWebSocket, RecordedRequest, Reply, Route, RouteHandler, RouteRequest (+2 more)

### Community 31 - "Live Profile Validation"
Cohesion: 0.13
Nodes (24): _canonical(), _covers(), _ends_with(), _index_of(), listing_filter(), _match_text(), _own_identity(), Perfume matching: brand and concentration are mandatory; fuzzy matching only app (+16 more)
...
Cohesion: 0.09
Nodes (27): _check_empty_search(), ExtractionFailed, RuntimeError, A page answered but gave up nothing, where something was expected.      This is, Fail when a search yielded no rows off a page that plainly lists products., FetchResult, One fetched page, uniform regardless of which strategy produced it., Check (+19 more)
...
Cohesion: 0.10
Nodes (28): ApiError, readDetail(), request(), Window, App(), Toast, View, UpdateDialog() (+20 more)
...
Cohesion: 0.16
Nodes (11): BasketResponse, BasketRow, BestCombination, SiteScenario, basket(), basketRow(), resultRow(), scenario() (+3 more)
...
Cohesion: 0.09
Nodes (38): add_basket_item(), now_iso(), Return the current UTC time as 'YYYY-MM-DDTHH:MM:SSZ'.      Every timestamp writ, Add a size of a perfume to the basket, and return the basket_item_id.      The p, Tests for parfum_finder.store: the timestamp helper and the schema.  The one har, A stale reading must never outrank the one taken after it.      latest_prices al, Whether an out-of-stock price counts as missing is the caller's call.      Dropp, Deleting a row that's already gone is a race between two screens, not a bug. (+30 more)
...
Cohesion: 0.15
Nodes (28): ParfumFinderApp, Path, Root app: pushes the search screen on mount., _ok_result(), LogCaptureFixture, MonkeyPatch, Submitting a query has to hand focus to the table.      A focused Input swallows, The row under the cursor has to survive the table being rebuilt.      Nothing re (+20 more)
...
Cohesion: 0.13
Nodes (22): Write one scan's reading of one size, and return its snapshot id.      The perfu, record_snapshot(), _counting_runner(), _days_ago(), Any, Runner, What the widget actually paints, with markup applied.      `.content` is the raw, Wait for the scan to end, which is the only time the table is filled.      Count (+14 more)
...
Cohesion: 0.08
Nodes (40): ProductCandidate, One hit on a search results page, before its product page is opened.      `raw_t, One decant size of one product, in the units the database stores.      Tenths of, A candidate together with the decant sizes its product page offers., What one site had to say about one query, and how much to trust it.      Four st, SearchHit, SiteResult, Variant (+32 more)
...
Cohesion: 0.15
Nodes (22): FastAPI, create_app(), encode_basket_refresh_event(), encode_basket_report(), encode_basket_row(), encode_result_row(), encode_scan_event(), encode_site_scenario() (+14 more)

### Community 82 - "cached_prices"
Cohesion: 0.33
...
### Community 83 - "connect"
Cohesion: 0.33
Nodes (6): connect(), Path, Open the price database, creating the schema if it isn't there yet.      Foreign, _basket_count(), The confirm dialog has to look and behave like something answerable.      Its ke, test_the_low_confidence_dialog_can_be_answered_with_the_keyboard_alone()
...
### Community 89 - "test_connect_is_idempotent_on_an_existing_database"
Cohesion: 0.50
...
### Community 93 - "ws.ts"
Cohesion: 0.21
Nodes (8): api, authToken(), refusalReason(), streamUrl(), useEventStream(), compile(), FakeServer, installFakeServer()
...
- **Why does `SearchScreen` connect `TUI App Shell` to `TUI App & Screens`, `_ResultRow`, `Title Matcher`, `_trial`, `TUI Confirm Dialog`, `ResultRow`, `Product Extraction`, `Search TUI Screen`, `conftest.py`, `Candidate Filtering`, `Price/Size Normalization`, `Static`, `._apply_scan_event`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `load_site_profile()` connect `Site Profiles & Templates` to `TUI App & Screens`, `Search Engine per Site`, `TUI Confirm Dialog`, `CLI Entry Points`, `TUI App Shell`, `Product Extraction`, `Profile Age Checks`, `Search TUI Screen`, `FetchResult`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `extract_jsonld_products()` connect `JSON-LD Product Extraction` to `validate_live`, `extract_embedded_variants`, `JsonLdProduct`, `Search Engine Core`, `._build_rows`?**
