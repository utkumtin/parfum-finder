...
## Corpus Check
- 103 files · ~282,800 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2218 nodes · 6337 edges · 82 communities (79 shown, 3 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 441 edges (avg confidence: 0.55)
...
Cohesion: 0.05
Nodes (85): _check_hook_kinds(), deep_merge(), _load_json(), load_platform_template(), load_platform_templates(), load_site_hooks(), load_site_profile(), Any (+77 more)

### Community 2 - "Title Matcher"
Cohesion: 0.06
Nodes (74): _Change, Lock, Protocol, What one site had to say about one query, and how much to trust it.      Four st, What a caller needs of run_site, as a type callers can stand a fake in for., SiteResult, SiteRunner, Fetcher (+66 more)
...
Cohesion: 0.06
Nodes (132): Screen, ProductCandidate, One hit on a search results page, before its product page is opened.      `raw_t, One decant size of one product, in the units the database stores.      Tenths of, A candidate together with the decant sizes its product page offers., SearchHit, Variant, connect() (+124 more)
...
Cohesion: 0.09
Nodes (40): CacheKey, _fetch_page(), _headers(), _is_excluded(), _paced_fetcher(), _page_offers_sizes(), _page_says_sold_out(), _parse_endpoint_document() (+32 more)
...
Cohesion: 0.13
Nodes (23): _classify_single_separator(), format_price(), _parse_number(), parse_price(), parse_size_ml(), Decimal, Number parsing and formatting for prices and volumes, plus text folding.  This i, Decide whether a lone separator marks a fraction or a thousands group.      Retu (+15 more)
...
Cohesion: 0.10
Nodes (35): FastAPI, create_app(), HTTP/WS backend for the GUI frontend. See api/app.py for the app itself., encode_basket_refresh_event(), encode_basket_report(), encode_basket_row(), encode_result_row(), encode_scan_event() (+27 more)

### Community 39 - "TUI App Shell"
Cohesion: 0.08
Nodes (12): Changed, HeaderSelected, RowSelected, ResultRow, Row, The initial screen: search bar, streaming results table, notices, footer., Close out a submit that named no perfume anyone could look for., Show what storage already knows, then go to the shops for the rest.          `fo (+4 more)
...
Cohesion: 0.25
Nodes (8): extract_jsonld_variants(), _flatten_jsonld(), Rung 1: read the page's JSON-LD as flat variant rows.      A product that declar, Turn one Product and everything under it into rows., test_jsonld_variants_collapses_a_size_listed_twice(), test_jsonld_variants_keeps_the_offers_nested_in_a_range(), test_jsonld_variants_refuses_a_price_range(), test_jsonld_variants_reports_each_size_and_not_the_parent()
...
Cohesion: 0.21
Nodes (11): _heading(), _leg_block(), BasketReport, Prices, SplitLeg, A block title plus the blank line that keeps it off the block above it.      The, The one line that says the screen is holding something back, or is not., One site's share of a split: its assigned items, then its own subtotal.      A s (+3 more)
...
Cohesion: 0.10
Nodes (25): _balanced_value(), _embedded_documents(), extract_embedded_variants(), extract_endpoint_variants(), _loads_or_skip(), Any, Rung 2: read the variant list out of a platform's JSON response.      `document`, Rung 3: read the JSON blob the page carries but does not display.      Two shape (+17 more)
...
Cohesion: 0.17
Nodes (15): _css_variant(), extract_css_variants(), _parse_selector(), Node, Rung 4: read the rendered markup with selectors. Last resort.      `config["vari, Read one variant's fields out of its container node., Run one "<css>::text" / "<css>::attr(name)" selector inside a node.      The nod, Run one "<css>::text" / "<css>::attr(name)" selector, reading every match. (+7 more)
...
Cohesion: 0.12
Nodes (26): HTMLParser, _check_empty_search(), _check_variant_control(), ExtractionFailed, RuntimeError, Read the result rows off a search page.      Every site needs selectors here, wh, A page answered but gave up nothing, where something was expected.      This is, Run one query against one site and read every hit's sizes.      Everything site- (+18 more)
...
Cohesion: 0.29
Nodes (7): CandidateFilter, _candidates_to_open(), Path, Run every site against one query, all at once, and report each separately., Narrow the search results down to the pages worth a request.      The first one, run_sites(), test_no_sites_is_an_empty_run_not_a_crash()
...
- **Why does `PerfumeQuery` connect `Search Engine per Site` to `_ResultRow`, `Title Matcher`, `TUI App Shell`, `Headers`, `Search TUI Screen`, `FetchResult`, `conftest.py`, `Candidate Filtering`, `Store Timestamp Tests`, `Live Profile Validation`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Why does `SearchScreen` connect `TUI App Shell` to `TUI App & Screens`, `Site Profiles & Templates`, `Title Matcher`, `Search/Basket Domain Models`, `Search Engine per Site`, `Headers`, `ScanStatus.tsx`, `Search TUI Screen`, `FetchResult`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `BasketScreen` connect `Title Matcher` to `TUI App & Screens`, `Site Profiles & Templates`, `TUI Confirm Dialog`, `TUI App Shell`, `Headers`, `Basket Optimizer Core`, `JsonLdProduct`, `ScanStatus.tsx`, `FetchResult`?**
