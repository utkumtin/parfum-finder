...
## Corpus Check
- 68 files · ~253,979 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1731 nodes · 4707 edges · 71 communities (68 shown, 3 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 190 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `064ae618`
...
Cohesion: 0.07
Nodes (98): LogCaptureFixture, Pressed, One decant size of one product, in the units the database stores.      Tenths of, A candidate together with the decant sizes its product page offers., What one site had to say about one query, and how much to trust it.      Four st, SearchHit, SiteResult, Variant (+90 more)

### Community 5 - "Search Engine per Site"
Cohesion: 0.17
Nodes (11): Split one typed line into the perfumes it asks for, on " - ".      The separator, split_queries(), _listing_filter(), The search screen: a progress bar while scanning, one grouped table after.  Colu, Decide, from a search result's own title, whether to open its page., test_a_hyphen_inside_a_brand_name_does_not_split_the_search(), test_a_line_naming_three_perfumes_is_read_as_three_searches(), test_a_piece_that_names_no_perfume_survives_to_be_complained_about() (+3 more)
...
Cohesion: 0.07
Nodes (58): HTMLParser, _check_variant_control(), _fetch_page(), _headers(), _is_excluded(), _paced_fetcher(), _page_offers_sizes(), _page_says_sold_out() (+50 more)
...
Cohesion: 0.06
Nodes (21): Changed, HeaderSelected, RowSelected, Any, CacheKey, CandidateFilter, Decimal, Row (+13 more)
...
Cohesion: 0.13
Nodes (22): The Textual App root. Handles screen navigation and is the app's default entry p, _age_of(), live_query(), _path(), profile_age_days(), datetime, Profile staleness checks. Two modes: offline (against saved fixtures) and live (, A URL's path with no trailing slash, for comparing two spellings of one page. (+14 more)
...
Cohesion: 0.09
Nodes (22): curl_cffi, httpx, playwright, type, additionalProperties, type, body, request_headers (+14 more)
...
Cohesion: 0.15
Nodes (20): _candidates_to_open(), _check_empty_search(), ExtractionFailed, CandidateFilter, RuntimeError, A page answered but gave up nothing, where something was expected.      This is, Run one query against one site and read every hit's sizes.      Everything site-, Narrow the search results down to the pages worth a request.      The first one (+12 more)
...
Cohesion: 0.18
Nodes (14): _listing_filter(), Any, CandidateFilter, Connection, Command-line entry point.  Subcommands will be added incrementally as the projec, Scan every perfume against every site and print each site as it lands.      One, Decide, from a search result's own title, whether to open its page., Write one site's rows in a transaction of its own.      Per site rather than per (+6 more)
...
Cohesion: 0.15
Nodes (23): add_basket_item(), now_iso(), _perfume_id(), _product_id(), Connection, SQLite persistence: an append-only price history.  Tables: sites, perfumes, prod, Return the current UTC time as 'YYYY-MM-DDTHH:MM:SSZ'.      Every timestamp writ, Write one scan's reading of one size, and return its snapshot id.      The perfu (+15 more)
...
### Community 63 - "MonkeyPatch"
Cohesion: 0.67
Nodes (3): MonkeyPatch, Record every delay the engine asks for instead of serving it.      Waiting for r, slept()
...
Cohesion: 0.12
Nodes (19): basket_prices(), Return the basket price matrix: one row per (line, site) that has a price., One call has to leave a row the search table can read straight off.      The cal, The old price has to survive, and it must not become a second variant.      Appe, first_seen is what says how long a shop has carried a size., A sold-out size often shows no price at all, and 0 would mean free.      Writing, Sites come from the profiles, so an id nothing synced is a mistake., A stale reading must never outrank the one taken after it.      latest_prices al (+11 more)

### Community 72 - "_NoRootParser"
Cohesion: 0.24
Nodes (11): FetchResult, One fetched page, uniform regardless of which strategy produced it., _LayerUnavailable, Exception, This profile carries no configuration for the layer being probed., _counting_fetcher(), Exception, Answer each call with the next canned result, then repeat the last one. (+3 more)

### Community 75 - "set_basket_qty"
Cohesion: 0.09
Nodes (22): conn(), Path, Tests for parfum_finder.store: the timestamp helper and the schema.  The one har, An update aimed at a row that isn't there means the caller is out of sync., A shop's imitation must not be stored as the perfume it imitates.      The clone, A snapshot pointing at a variant that doesn't exist has to be rejected.      SQL, Reopening an existing database must not wipe or re-raise on its schema., Another house's bottle on the same results page must not enter this history. (+14 more)
...
- **Why does `SiteResult` connect `Search/Basket Domain Models` to `TUI App & Screens`, `Title Matcher`, `HTTP/Browser Fetching`, `Search Engine per Site`, `TUI Confirm Dialog`, `TUI App Shell`, `_NoRootParser`, `Search Engine Core`, `set_basket_qty`, `Multi-Site Search Run`, `Basket TUI Screen`, `Basket Domain Logic`, `Search TUI Screen`, `conftest.py`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `SearchScreen` connect `Search TUI Screen` to `TUI App & Screens`, `_ResultRow`, `Site Profiles & Templates`, `Search/Basket Domain Models`, `Search Engine per Site`, `TUI Confirm Dialog`, `TUI App Shell`, `Search Engine Core`, `Basket TUI Screen`, `Live Profile Validation`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `BasketScreen` connect `Basket TUI Screen` to `TUI App & Screens`, `_ResultRow`, `Title Matcher`, `Search/Basket Domain Models`, `Search Engine per Site`, `TUI Confirm Dialog`, `TUI App Shell`, `Basket Optimizer Core`, `Search TUI Screen`, `Price/Size Normalization`, `Fetch Strategy Selection`?**
...
- **Should `TUI App & Screens` be split into smaller, more focused modules?**
  _Cohesion score 0.09636363636363636 - nodes in this community are weakly interconnected._