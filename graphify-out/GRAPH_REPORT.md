...
## Corpus Check
- 137 files · ~317,824 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2621 nodes · 7467 edges · 98 communities (94 shown, 4 thin omitted)
...
Cohesion: 0.11
Nodes (29): Connection, The basket screen prints brand/name/concentration straight off this row.      Or, Two lines added within the same second must still read back the same way twice., A basket line nobody sells must still be visible via basket_lines.      basket_p, Whether an out-of-stock price counts as missing is the caller's call.      Dropp, Deleting a row that's already gone is a race between two screens, not a bug., The table's CHECK (qty > 0) would reject a bare 0, and the '-' key has to     su, Insert one site → perfume → product → variant chain, return the variant id. (+21 more)
...
Cohesion: 0.13
Nodes (23): add_basket_item(), _perfume_id(), price_history(), _product_id(), Connection, Row, Write one scan's reading of one size, and return its snapshot id.      The perfu, Write a whole scan at once and return how many prices were recorded.      Every (+15 more)
...
Cohesion: 0.12
Nodes (49): Lock, _record_search(), _sync_profiles(), listing_filter(), Decide, from a search result's own title, whether to open its page.      Structu, Any, BasketRow, A site's display name, with a badge when its profile is old enough     to be wor (+41 more)
...
Cohesion: 0.10
Nodes (13): _Change, _remove_basket_item(), _set_basket_qty(), Delete one basket line, and say whether there was one to delete.      Returns Fa, Set a basket line's quantity, clamped to at least 1, and return it.      The tab, remove_basket_item(), set_basket_qty(), BasketScreen (+5 more)
...
Cohesion: 0.07
Nodes (59): Pressed, ScanEvent, BasketRefreshEvent and viewmodel dataclasses as JSON-safe dicts.  Eve, _paced_fetcher(), One site's pacing state, for as long as whoever holds it says.      The gate and, Wrap a fetcher so one site's requests go out one at a time, spaced apart.      T, SitePace, Fetcher, Protocol (+51 more)
...
Cohesion: 0.07
Nodes (44): encode_basket_report(), encode_basket_row(), encode_result_row(), encode_scan_event(), encode_site_scenario(), _encode_split_leg(), encode_split_plan(), Any (+36 more)
...
- **Why does `UpdateDownload` connect `Fixture Fetcher (Tests)` to `validate_live`, `JsonLdProduct`, `Basket Store & Pricing`, `_ResultRow`, `_FakeStreamResponse`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `discover()` connect `Platform Discovery Flow` to `Site Profiles & Templates`, `extract_embedded_variants`, `CLI Entry Points`, `._build_rows`, `Playwright Errors`, `enum`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `PerfumeQuery` connect `Offline Profile Validation` to `_ResultRow`, `.on_mount`, `_named_profile`, `CLI Entry Points`, `Fixture Fetcher (Tests)`, `run_sites`, `_RecordingFetcher`, `Search TUI Screen`, `FetchResult`, `._start_search`, `_seed_site`, `Live Profile Validation`?**
