...
Cohesion: 0.05
Nodes (135): Screen, ProductCandidate, One hit on a search results page, before its product page is opened.      `raw_t, One decant size of one product, in the units the database stores.      Tenths of, A candidate together with the decant sizes its product page offers., SearchHit, Variant, Write one scan's reading of one size, and return its snapshot id.      The perfu (+127 more)

### Community 5 - "Search Engine per Site"
Cohesion: 0.10
Nodes (30): _add_basket_item(), _load_profiles(), Any, Path, _recent_searches(), _remove_basket_item(), _set_basket_qty(), _site_summary() (+22 more)
...
Cohesion: 0.07
Nodes (57): HTMLParser, _check_variant_control(), _fetch_page(), _headers(), _is_excluded(), _jitter_s(), _paced_fetcher(), _page_evidence() (+49 more)
...
Cohesion: 0.14
Nodes (22): CandidateFilter, _candidates_to_open(), _check_empty_search(), ExtractionFailed, RuntimeError, A page answered but gave up nothing, where something was expected.      This is, Run one query against one site and read every hit's sizes.      Everything site-, Narrow the search results down to the pages worth a request.      The first one (+14 more)
...
Cohesion: 0.09
Nodes (61): Run one site and classify what came back instead of raising.      It is also whe, run_site(), FetchResult, One fetched page, uniform regardless of which strategy produced it., _counting_fetcher(), _profile(), Exception, Tests for the profile-driven search in parfum_finder.engine.  What these defend (+53 more)

### Community 18 - "Playwright Errors"
Cohesion: 0.13
Nodes (22): Command-line entry point.  Subcommands will be added incrementally as the projec, _attempt(), _count_jsonld(), _count_product_objects(), _detect_platforms(), format_report(), _label_platform(), _one_line() (+14 more)
...
Cohesion: 0.09
Nodes (57): match_title(), parse_query(), PerfumeQuery, The perfume being looked for, split into its three identity parts.      `concent, Split one typed line, "Dior Sauvage EDP", into the three identity parts.      Th, Judge one site title against a query, or None if it is not that perfume.      No, Whether a search result's own listing text is worth opening the page for.      J, title_could_match() (+49 more)
...
### Community 37 - "_trial"
Cohesion: 0.12
Nodes (23): _classify_single_separator(), format_age(), format_ml(), _parse_number(), parse_price(), parse_size_ml(), Decimal, Number parsing and formatting for prices and volumes, plus text folding.  This i (+15 more)

### Community 38 - "TUI Confirm Dialog"
Cohesion: 0.11
Nodes (53): BaseModel, AcceptedSearch, _AppState, BasketAddRequest, BasketQtyRequest, The FastAPI app: a thin HTTP/WS wrapper around the Faz 1 services.  No business, _read_basket(), RefreshRequest (+45 more)
...
Cohesion: 0.12
Nodes (52): TestClient, _auth(), _client(), db_path(), _ok_result(), Any, MonkeyPatch, Path (+44 more)
...
Cohesion: 0.12
Nodes (18): Any, Connection, Scan every perfume against every site and print each site as it lands.      One, Write one site's rows in a transaction of its own.      Per site rather than per, One line per site: which site, how it went, and what it said.      The site id i, _report_line(), _scan_all(), _store_site_result() (+10 more)
...
Cohesion: 0.24
Nodes (10): BasketRow, _score_basket(), basket_inputs(), BasketRow, Prices, Score every enabled site against the whole basket and sort the results.      Sit, The three inputs site_scenario/optimize score: items, prices, shipping.      Bui, One site's shipping terms, read once and reused for every scenario.      `free_s (+2 more)
...
Cohesion: 0.09
Nodes (54): Pressed, Protocol, What one site had to say about one query, and how much to trust it.      Four st, One site's pacing state, for as long as whoever holds it says.      The gate and, What a caller needs of run_site, as a type callers can stand a fake in for., SitePace, SiteResult, SiteRunner (+46 more)
...
### Community 95 - "Static"
Cohesion: 0.38
Nodes (7): _basket_price(), _days_ago(), _line(), A row is only as fresh as the oldest price it is comparing.      Reporting the n, test_a_rows_age_is_its_stalest_cell(), test_basket_inputs_builds_the_price_matrix_from_row_data(), test_build_basket_rows_drops_out_of_stock_and_excluded_prices()

### Community 96 - ".__init__"
Cohesion: 0.33
Nodes (6): CacheKey, Path, Run every site against one query, all at once, and report each separately., run_sites(), test_no_sites_is_an_empty_run_not_a_crash(), VariantsRead
...
- **Why does `SearchScreen` connect `TUI App Shell` to `TUI App & Screens`, `_ResultRow`, `Search/Basket Domain Models`, `_trial`, `TUI Confirm Dialog`, `run_sites`, `Search TUI Screen`, `Candidate Filtering`, `_FixtureFetcher`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Why does `PlaywrightNotInstalled` connect `HTTP/Browser Fetching` to `Search/Basket Domain Models`, `Search Engine Core`, `Basket Store & Pricing`, `FieldConfidence`, `._build_rows`, `run_sites`, `Playwright Errors`, `test_connect_is_idempotent_on_an_existing_database`, `Offline Profile Validation`, `SQLite Store`, `_named_profile`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `SiteRunner` connect `run_sites` to `_retry_after_s`, `Title Matcher`, `HTTP/Browser Fetching`, `Search/Basket Domain Models`, `TUI Confirm Dialog`, `TUI App Shell`, `Search Engine Core`, `_ResultRow`, `Offline Profile Validation`, `Candidate Filtering`, `_FixtureFetcher`?**
