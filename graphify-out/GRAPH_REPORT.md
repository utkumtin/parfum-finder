...
Cohesion: 0.14
Nodes (14): add_basket_item(), Add a size of a perfume to the basket, and return the basket_item_id.      The p, The basket screen prints brand/name/concentration straight off this row.      Or, A basket line nobody sells must still be visible via basket_lines.      basket_p, A 10 ml listing must never fill a basket line asking for 5 ml.      The matrix j, Whether an out-of-stock price counts as missing is the caller's call.      Dropp, The table's CHECK (qty > 0) would reject a bare 0, and the '-' key has to     su, A basket line for a perfume nobody has priced is a bug, not a state to keep. (+6 more)
...
Cohesion: 0.12
Nodes (44): basket_inputs(), BasketItem, optimize(), BasketRow, Prices, Score one site against the basket, or against a subset of it.      `item_ids` is, Score every enabled site against the whole basket and sort the results.      Sit, Search for the cheapest way to split the basket across several sites.      Retur (+36 more)

### Community 11 - "Basket Store & Pricing"
Cohesion: 0.13
Nodes (20): BasketRefreshSession, In-memory session state for the two streamed operations: a search scan and a bas, ScanSession, File logging for the app's own diagnostics.  Nothing here ever writes to the con, Pure sorting and grouping rules for the results table.  No I/O, no Textual state, _about(), _cached_result_row(), ResultRow (+12 more)
...
Cohesion: 0.11
Nodes (30): _recent_searches(), _record_search(), _remove_basket_item(), _set_basket_qty(), _perfume_id(), _product_id(), Connection, SQLite persistence: an append-only price history.  Tables: sites, perfumes, prod (+22 more)
...
Cohesion: 0.22
Nodes (10): Collection, build_basket_rows(), _label(), Name one basket line the way a missing-item warning has to read it., Turn basket lines and their site prices into what the table shows.      Out of s, Whole days between a price snapshot and now.      Only the one UTC format now_is, snapshot_age_days(), _days_ago() (+2 more)
...
### Community 37 - "_trial"
Cohesion: 0.09
Nodes (36): _classify_single_separator(), format_age(), format_ml(), format_price(), _parse_number(), parse_price(), parse_size_ml(), Decimal (+28 more)

### Community 38 - "TUI Confirm Dialog"
Cohesion: 0.13
Nodes (39): BaseModel, AcceptedSearch, _add_basket_item(), _AppState, BasketAddRequest, BasketQtyRequest, _load_profiles(), Any (+31 more)
...
Cohesion: 0.08
Nodes (67): FastAPI, create_app(), SiteRunner, UpdateDownload, HTTP/WS backend for the GUI frontend. See api/app.py for the app itself., TestClient, _auth(), _client() (+59 more)
...
Cohesion: 0.11
Nodes (28): datetime, conn(), Path, Tests for parfum_finder.store: the timestamp helper and the schema.  The one har, Two lines added within the same second must still read back the same way twice., Deleting a row that's already gone is a race between two screens, not a bug., Insert one site → perfume → product → variant chain, return the variant id., Two snapshots written in the same second must resolve to the newer one.      A s (+20 more)
...
Cohesion: 0.19
Nodes (36): Lock, Connection, Mirror site profiles into the sites table and return how many were written., sync_to_db(), Show what storage already knows, then go to the shops for the rest.      `force=, run_scan(), now_iso(), Return the current UTC time as 'YYYY-MM-DDTHH:MM:SSZ'.      Every timestamp writ (+28 more)
...
Cohesion: 0.14
Nodes (9): _Change, BasketScreen, BasketReport, BasketRow, The one line that says the screen is holding something back, or is not., The basket: the list on top, one scenario per site underneath., _remove(), _set_qty() (+1 more)
...
Cohesion: 0.15
Nodes (18): encode_split_plan(), The split plan plus its verdict against the best full-coverage site.      Named, BasketReport, compare_split_to_best_full(), Basket scenario evaluation. A pure function, no network access, no sqlite.  Inpu, Every site's single-site scenario, split by whether it covers everything.      A, One site's share of a split basket: what to buy there and what it costs.      `s, The cheapest basket split the search found. A heuristic, not a proof.      Every (+10 more)
...
Cohesion: 0.11
Nodes (47): Protocol, What one site had to say about one query, and how much to trust it.      Four st, One site's pacing state, for as long as whoever holds it says.      The gate and, What a caller needs of run_site, as a type callers can stand a fake in for., SitePace, SiteResult, SiteRunner, BasketPriceExcluded (+39 more)
...
Cohesion: 0.14
Nodes (14): Connection, A disabled site loses its basket column, but an enabled quiet one keeps one., NULL means the site has no free shipping tier at all, not a threshold of zero., An update aimed at a row that isn't there means the caller is out of sync., The recents list has five slots, so a repeat must not consume two.      Someone, A snapshot pointing at a variant that doesn't exist has to be rejected.      SQL, test_basket_sites_omits_a_disabled_site_and_keeps_one_that_prices_nothing(), test_basket_sites_preserves_a_null_free_shipping_threshold_as_none() (+6 more)
...
## Knowledge Gaps
- **226 isolated node(s):** `View`, `Toast`, `Window`, `Notice`, `TRIAL_SIZES_ML_X10` (+221 more)
...
- **Why does `connect()` connect `Search/Basket Domain Models` to `TUI App & Screens`, `Site Profiles & Templates`, `_trial`, `TUI Confirm Dialog`, `CLI Entry Points`, `Basket Store & Pricing`, `run_sites`, `_RecordingFetcher`, `FetchResult`, `Candidate Filtering`, `Basket Site Scenarios`, `cli.py`, `_FixtureFetcher`?**
...
- **Why does `SearchScreen` connect `Basket Site Scenarios` to `TUI App & Screens`, `_ResultRow`, `Search/Basket Domain Models`, `Static`, `Basket Store & Pricing`, `run_sites`, `Search TUI Screen`, `_FixtureFetcher`?**
