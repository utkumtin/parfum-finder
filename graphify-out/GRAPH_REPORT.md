...
## Summary
- 2163 nodes · 6182 edges · 79 communities (74 shown, 5 thin omitted)
...
Cohesion: 0.06
Nodes (124): Screen, ProductCandidate, One hit on a search results page, before its product page is opened.      `raw_t, One decant size of one product, in the units the database stores.      Tenths of, A candidate together with the decant sizes its product page offers., SearchHit, Variant, connect() (+116 more)
...
Cohesion: 0.08
Nodes (44): conn(), Connection, Path, Tests for parfum_finder.store: the timestamp helper and the schema.  The one har, A 10 ml listing must never fill a basket line asking for 5 ml.      The matrix j, A disabled site loses its basket column, but an enabled quiet one keeps one., NULL means the site has no free shipping tier at all, not a threshold of zero., Deleting a row that's already gone is a race between two screens, not a bug. (+36 more)
...
Cohesion: 0.18
Nodes (39): TestClient, _auth(), _client(), db_path(), _ok_result(), Any, MonkeyPatch, Path (+31 more)
...
Cohesion: 0.13
Nodes (44): BaseModel, AcceptedSearch, _add_basket_item(), _AppState, BasketAddRequest, BasketQtyRequest, _load_profiles(), Any (+36 more)

### Community 53 - "conftest.py"
Cohesion: 0.11
Nodes (30): add_basket_item(), basket_prices(), cached_prices(), _perfume_id(), _product_id(), Connection, datetime, SQLite persistence: an append-only price history.  Tables: sites, perfumes, prod (+22 more)
...
- **Why does `SearchScreen` connect `._refresh_table` to `TUI App & Screens`, `Title Matcher`, `Search/Basket Domain Models`, `_trial`, `TUI Confirm Dialog`, `Search TUI Screen`, `FetchResult`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `PlaywrightNotInstalled` connect `HTTP/Browser Fetching` to `Fetch Strategy Probing`, `Search/Basket Domain Models`, `Search Engine per Site`, `TUI Confirm Dialog`, `Search Engine Core`, `_LayerUnavailable`, `FieldConfidence`, `BasketRefreshEvent`, `._build_rows`, `Offline Profile Validation`, `Playwright Errors`, `Basket Site Scenarios`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `SiteRunner` connect `TUI Confirm Dialog` to `Site Profiles & Templates`, `Title Matcher`, `test_cached_prices_is_empty_for_a_perfume_nobody_scanned`, `HTTP/Browser Fetching`, `Search/Basket Domain Models`, `exclude_keywords`, `_LayerUnavailable`, `Search Engine Core`, `_ResultRow`, `JsonLdProduct`, `._refresh_table`, `FetchResult`, `Snapshot Writing`?**
