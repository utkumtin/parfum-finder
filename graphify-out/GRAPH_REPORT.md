...
## Corpus Check
- 125 files · ~305,960 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2465 nodes · 6908 edges · 89 communities (86 shown, 3 thin omitted)
...
Cohesion: 0.09
Nodes (54): Pressed, ScanEvent, BasketRefreshEvent and viewmodel dataclasses as JSON-safe dicts.  Eve, Protocol, What one site had to say about one query, and how much to trust it.      Four st, What a caller needs of run_site, as a type callers can stand a fake in for., SiteResult, SiteRunner, _about() (+46 more)
...
Cohesion: 0.06
Nodes (132): Screen, One decant size of one product, in the units the database stores.      Tenths of, A candidate together with the decant sizes its product page offers., SearchHit, Variant, connect(), Path, Open the price database, creating the schema if it isn't there yet.      Foreign (+124 more)
...
Cohesion: 0.09
Nodes (46): HTMLParser, _check_variant_control(), _fetch_page(), _headers(), _paced_fetcher(), _page_offers_sizes(), _page_says_sold_out(), _parse_endpoint_document() (+38 more)
...
### Community 37 - "_trial"
Cohesion: 0.08
Nodes (35): _classify_single_separator(), format_age(), format_ml(), format_price(), _parse_number(), parse_price(), parse_size_ml(), Decimal (+27 more)

### Community 38 - "TUI Confirm Dialog"
Cohesion: 0.08
Nodes (70): BaseModel, FastAPI, AcceptedSearch, _add_basket_item(), _AppState, BasketAddRequest, BasketQtyRequest, create_app() (+62 more)
...
- **Why does `SearchScreen` connect `TUI App Shell` to `TUI App & Screens`, `_ResultRow`, `Title Matcher`, `Search/Basket Domain Models`, `TUI Confirm Dialog`, `Search TUI Screen`, `Candidate Filtering`, `_FixtureFetcher`?**
  _High betweenness centrality (0.052) - this node is a cross-community bridge._
- **Why does `BasketScreen` connect `_FixtureFetcher` to `TUI App & Screens`, `Site Profiles & Templates`, `Title Matcher`, `TUI Confirm Dialog`, `TUI App Shell`, `Basket Optimizer Core`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `SiteResult` connect `Title Matcher` to `TUI App & Screens`, `Fetch Strategy Probing`, `Search/Basket Domain Models`, `Search Engine per Site`, `TUI Confirm Dialog`, `Search Engine Core`, `FieldConfidence`, `SplitPlan`, `_ResultRow`, `Offline Profile Validation`, `FetchResult`, `Candidate Filtering`?**
