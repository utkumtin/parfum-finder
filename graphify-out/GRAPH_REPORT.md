...
## Corpus Check
- 68 files · ~254,709 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1738 nodes · 4733 edges · 71 communities (69 shown, 2 thin omitted)
...
Cohesion: 0.07
Nodes (101): LogCaptureFixture, Pressed, One decant size of one product, in the units the database stores.      Tenths of, A candidate together with the decant sizes its product page offers., What one site had to say about one query, and how much to trust it.      Four st, SearchHit, SiteResult, Variant (+93 more)
...
Cohesion: 0.11
Nodes (15): _Change, BasketLine, BasketPrice, One row of the basket: a size of a perfume, with the identity spelled out., One site's latest price for one basket line, only when it has one.      Rows wit, _BasketRow, BasketScreen, _label() (+7 more)
...
Cohesion: 0.11
Nodes (28): basket_lines(), basket_prices(), basket_sites(), BasketSite, _perfume_id(), _product_id(), Connection, SQLite persistence: an append-only price history.  Tables: sites, perfumes, prod (+20 more)
...
Cohesion: 0.28
Nodes (3): Changed, One perfume of a search, as typed and as parsed.      The index is the outermost, _Search
...
- **Why does `SiteResult` connect `Search/Basket Domain Models` to `TUI App & Screens`, `Title Matcher`, `TUI Confirm Dialog`, `CLI Entry Points`, `TUI App Shell`, `Search Engine Core`, `Basket TUI Screen`, `Basket Domain Logic`, `Profile Age Checks`, `._refresh_table`, `Snapshot Writing`, `conftest.py`, `Basket Site Scenarios`, `SQLite Store`, `ComposeResult`, `Live Profile Validation`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Why does `BasketScreen` connect `Basket TUI Screen` to `TUI App & Screens`, `_ResultRow`, `Title Matcher`, `Search/Basket Domain Models`, `TUI Confirm Dialog`, `TUI App Shell`, `Basket Optimizer Core`, `._refresh_table`, `conftest.py`, `Basket Site Scenarios`, `tui/__init__.py`, `ComposeResult`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `PerfumeQuery` connect `_ResultRow` to `Title Matcher`, `Search/Basket Domain Models`, `TUI Confirm Dialog`, `CLI Entry Points`, `TUI App Shell`, `Basket TUI Screen`, `._refresh_table`, `conftest.py`, `Basket Site Scenarios`, `ComposeResult`, `Live Profile Validation`?**
