...
## Summary
- 2497 nodes · 7040 edges · 99 communities (93 shown, 6 thin omitted)
...
Cohesion: 0.10
Nodes (30): _load_profiles(), Any, Path, _recent_searches(), _record_search(), _remove_basket_item(), _set_basket_qty(), _site_summary() (+22 more)
...
Cohesion: 0.15
Nodes (41): BaseModel, AcceptedSearch, _add_basket_item(), _AppState, BasketAddRequest, BasketQtyRequest, The FastAPI app: a thin HTTP/WS wrapper around the Faz 1 services.  No business, _read_basket() (+33 more)
...
Cohesion: 0.09
Nodes (21): _Change, BasketScreen, _heading(), _leg_block(), BasketReport, Path, Prices, SiteScenario (+13 more)
...
Cohesion: 0.07
Nodes (70): Lock, Pressed, In-memory session state for the two streamed operations: a search scan and a bas, _paced_fetcher(), Protocol, What one site had to say about one query, and how much to trust it.      Four st, One site's pacing state, for as long as whoever holds it says.      The gate and, Wrap a fetcher so one site's requests go out one at a time, spaced apart.      T (+62 more)
...
- **Why does `SearchScreen` connect `TUI App Shell` to `test_an_unreadable_tag_never_counts_as_an_update`, `TUI App & Screens`, `Search/Basket Domain Models`, `Search Engine per Site`, `TUI Confirm Dialog`, `_trial`, `ConfirmDialog.tsx`, `_collect_products`, `run_sites`, `Search TUI Screen`, `FetchResult`, `Candidate Filtering`, `_FixtureFetcher`, `._apply_scan_event`?**
...
- **Why does `SiteRunner` connect `run_sites` to `Title Matcher`, `HTTP/Browser Fetching`, `Search/Basket Domain Models`, `TUI Confirm Dialog`, `TUI App Shell`, `Search Engine Core`, `ConfirmDialog.tsx`, `_ResultRow`, `Offline Profile Validation`, `Candidate Filtering`, `_FixtureFetcher`?**
