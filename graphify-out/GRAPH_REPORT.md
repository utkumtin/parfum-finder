...
## Corpus Check
- 126 files · ~310,620 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2505 nodes · 7047 edges · 98 communities (94 shown, 4 thin omitted)
...
Cohesion: 0.08
Nodes (41): _load_profiles(), Any, Path, _recent_searches(), _record_search(), _remove_basket_item(), _set_basket_qty(), _site_summary() (+33 more)
...
Cohesion: 0.07
Nodes (60): CacheKey, HTMLParser, _check_variant_control(), _fetch_page(), _headers(), _jitter_s(), _paced_fetcher(), _page_evidence() (+52 more)
...
Cohesion: 0.08
Nodes (35): _age_of(), Check, _count_result_cards(), _first_result_url(), live_query(), _no_results_check(), _path(), _probe_layer() (+27 more)
...
Cohesion: 0.11
Nodes (27): Pattern, _brand_pattern(), _canonical(), _canonical_brands(), _covers(), _ends_with(), _index_of(), _match_text() (+19 more)
...
Cohesion: 0.12
Nodes (49): BaseModel, AcceptedSearch, _add_basket_item(), _AppState, BasketAddRequest, BasketQtyRequest, The FastAPI app: a thin HTTP/WS wrapper around the Faz 1 services.  No business, _read_basket() (+41 more)
...
Cohesion: 0.12
Nodes (24): conn(), Path, Tests for parfum_finder.store: the timestamp helper and the schema.  The one har, The table's CHECK (qty > 0) would reject a bare 0, and the '-' key has to     su, Insert one site → perfume → product → variant chain, return the variant id., Two snapshots written in the same second must resolve to the newer one.      A s, 5 ml at 125,00 TL is 25,00 TL per ml. Stored as kurus, ml as tenths., No price yet means no row, so the basket LEFT JOIN reads it as missing. (+16 more)

### Community 50 - "Profile Age Checks"
Cohesion: 0.16
Nodes (28): Check one site's profile against that site's saved fixtures.      Never raises f, Run one site's profile against the real site.      Same contract as offline mode, validate_live(), validate_offline(), _corrupted_sites_dir(), _DeadSite, _FakeSite, _fixture_site() (+20 more)
...
Cohesion: 0.11
Nodes (13): _Change, _label(), Name one basket line the way a missing-item warning has to read it., format_age(), format_ml(), Format a volume for display (dot-decimal): Decimal('1.5') -> '1.5 ml'., Turn a price age in days into the words the age column shows., BasketScreen (+5 more)
...
Cohesion: 0.25
Nodes (7): Blocking issue, Conclusion, Private GitHub releases are requested without authentication, Result, Verification, What is working correctly, Windows Update System Audit

### Community 75 - "ResultRow"
Cohesion: 0.09
Nodes (37): _as_str(), _balanced_value(), _build_offer(), _build_product(), _coerce_in_stock(), _collect_offers(), _collect_products(), _collect_variants() (+29 more)
...
Cohesion: 0.07
Nodes (85): Lock, Pressed, Protocol, What one site had to say about one query, and how much to trust it.      Four st, One site's pacing state, for as long as whoever holds it says.      The gate and, What a caller needs of run_site, as a type callers can stand a fake in for., SitePace, SiteResult (+77 more)
...
- **Why does `SearchScreen` connect `TUI App Shell` to `TUI App & Screens`, `_ResultRow`, `Search/Basket Domain Models`, `Search Engine per Site`, `TUI Confirm Dialog`, `run_sites`, `Search TUI Screen`, `_FixtureFetcher`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Why does `PlaywrightNotInstalled` connect `FieldConfidence` to `Search/Basket Domain Models`, `Search Engine Core`, `Basket Store & Pricing`, `._build_rows`, `run_sites`, `Playwright Errors`, `test_connect_is_idempotent_on_an_existing_database`, `Offline Profile Validation`, `SQLite Store`, `_named_profile`, `MonkeyPatch`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `SiteRunner` connect `run_sites` to `Title Matcher`, `Search/Basket Domain Models`, `TUI Confirm Dialog`, `TUI App Shell`, `Search Engine Core`, `FieldConfidence`, `_ResultRow`, `Offline Profile Validation`, `Price/Size Normalization`, `_FixtureFetcher`?**
