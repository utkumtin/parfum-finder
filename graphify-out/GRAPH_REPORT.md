...
## Corpus Check
- 103 files · ~279,524 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2193 nodes · 6222 edges · 83 communities (76 shown, 7 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 431 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2797bec9`
...
Cohesion: 0.08
Nodes (52): HTMLParser, _check_empty_search(), _check_variant_control(), _fetch_page(), _headers(), _is_excluded(), _paced_fetcher(), _page_offers_sizes() (+44 more)
...
Cohesion: 0.13
Nodes (15): A 10 ml listing must never fill a basket line asking for 5 ml.      The matrix j, Whether an out-of-stock price counts as missing is the caller's call.      Dropp, The old price has to survive, and it must not become a second variant.      Appe, The title and URL are information, not identity.      A shop that rewords a list, EDT and EDP are different products at different prices.      Folding them into o, A sold-out size often shows no price at all, and 0 would mean free.      Writing, Sites come from the profiles, so an id nothing synced is a mistake., _record() (+7 more)
...
Cohesion: 0.08
Nodes (75): BaseModel, Lock, AcceptedSearch, _AppState, BasketAddRequest, BasketQtyRequest, RefreshStartResponse, SearchRequest (+67 more)

### Community 39 - "TUI App Shell"
Cohesion: 0.12
Nodes (32): FastAPI, _add_basket_item(), create_app(), _load_profiles(), Any, Path, The FastAPI app: a thin HTTP/WS wrapper around the Faz 1 services.  No business, _remove_basket_item() (+24 more)
...
Cohesion: 0.08
Nodes (44): conn(), Connection, Path, Tests for parfum_finder.store: the timestamp helper and the schema.  The one har, A stale reading must never outrank the one taken after it.      latest_prices al, A disabled site loses its basket column, but an enabled quiet one keeps one., NULL means the site has no free shipping tier at all, not a threshold of zero., Deleting a row that's already gone is a race between two screens, not a bug. (+36 more)
...
Cohesion: 0.09
Nodes (43): Collection, _read_basket(), BasketReport, build_basket_rows(), _ClimbState, compare_split_to_best_full(), _label(), BasketRow (+35 more)
...
Cohesion: 0.13
Nodes (15): One call has to leave a row the search table can read straight off.      The cal, first_seen is what says how long a shop has carried a size., raw_title is the audit trail for a wrong match, so it cannot be blank.      A ro, A slow site must not spread one reading across several timestamps.      Rows wri, The number reported is what landed in the history, not what was offered., Half a site's sizes updated is worse than none of them.      The basket compares, The trend panel reads row 0 as the latest reading, so order is the point.      A, _seed_site() (+7 more)
...
## Knowledge Gaps
- **206 isolated node(s):** `Notice`, `Block`, `SORT_LABELS`, `parfum-finder`, `$schema` (+201 more)
...
- **Why does `BasketScreen` connect `Title Matcher` to `TUI App & Screens`, `Site Profiles & Templates`, `TUI Confirm Dialog`, `Basket Optimizer Core`, `._refresh_table`, `FetchResult`, `snapshot_rows`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Why does `PerfumeQuery` connect `_trial` to `_ResultRow`, `TUI Confirm Dialog`, `CLI Entry Points`, `Search TUI Screen`, `FetchResult`, `._refresh_table`, `Candidate Filtering`, `Store Timestamp Tests`, `Live Profile Validation`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `SearchScreen` connect `._refresh_table` to `TUI App & Screens`, `Title Matcher`, `Search/Basket Domain Models`, `_trial`, `TUI Confirm Dialog`, `Search TUI Screen`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
...
- **What connects `Notice`, `Block`, `SORT_LABELS` to the rest of the system?**
