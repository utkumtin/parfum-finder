...
## Corpus Check
- 106 files · ~287,551 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2320 nodes · 6580 edges · 81 communities (77 shown, 4 thin omitted)
...
Cohesion: 0.09
Nodes (52): Pressed, Protocol, What one site had to say about one query, and how much to trust it.      Four st, What a caller needs of run_site, as a type callers can stand a fake in for., SiteResult, SiteRunner, Fetcher, Protocol (+44 more)
...
Cohesion: 0.07
Nodes (58): CacheKey, CandidateFilter, HTMLParser, _candidates_to_open(), _check_variant_control(), _fetch_page(), _headers(), _is_excluded() (+50 more)

### Community 10 - "Basket Optimizer Core"
Cohesion: 0.08
Nodes (59): Collection, BasketRow, _score_basket(), basket_inputs(), BasketItem, build_basket_rows(), _label(), optimize() (+51 more)
...
Cohesion: 0.08
Nodes (72): ExtractionFailed, RuntimeError, A page answered but gave up nothing, where something was expected.      This is, Run one site and classify what came back instead of raising.      It is also whe, Run one query against one site and read every hit's sizes.      Everything site-, run_site(), search_site(), _counting_fetcher() (+64 more)

### Community 18 - "Playwright Errors"
Cohesion: 0.11
Nodes (26): Command-line entry point.  Subcommands will be added incrementally as the projec, _choose_strategy(), _qualifies(), Pick the cheapest strategy that came back with real content, or None.      probe, Whether one strategy came back with a usable page., _attempt(), _count_jsonld(), _count_product_objects() (+18 more)
...
Cohesion: 0.17
Nodes (35): Lock, Any, BasketRow, A site's display name, with a badge when its profile is old enough     to be wor, Show what storage already knows, then go to the shops for the rest.      `force=, Ask every enabled site for today's price of every basket row.      One task per, run_basket_refresh(), run_scan() (+27 more)
...
Cohesion: 0.07
Nodes (17): Changed, HeaderSelected, RowSelected, format_age(), format_price(), Format a price for display (comma-thousands, dot-decimal).      Decimal('1250'), Turn a price age in days into the words the age column shows., The Textual App root. Handles screen navigation and is the app's default entry p (+9 more)
...
Cohesion: 0.05
Nodes (53): Client, check_enabled(), check_for_update(), DownloadProgress, fetch_latest_release(), _installer_asset(), is_newer(), launch_installer() (+45 more)
...
Cohesion: 0.19
Nodes (12): Run one site's profile against the real site.      Same contract as offline mode, validate_live(), _DeadSite, _FakeSite, _fixture_site(), A stand-in for one live site, answering the search page then the rest.      Live, A host that cannot be reached at all., test_a_broken_layer_reports_which_other_layer_could_take_over() (+4 more)

### Community 53 - "conftest.py"
Cohesion: 0.10
Nodes (54): BaseModel, AcceptedSearch, _add_basket_item(), _AppState, BasketAddRequest, BasketQtyRequest, _load_profiles(), Any (+46 more)
...
Cohesion: 0.15
Nodes (22): _close_browser(), _fetch_curl_cffi(), _fetch_httpx(), _fetch_playwright(), FetchResult, _launch_browser(), PlaywrightNoResponse, Any (+14 more)
...
Cohesion: 0.11
Nodes (31): add_basket_item(), basket_prices(), _perfume_id(), _product_id(), Connection, datetime, SQLite persistence: an append-only price history.  Tables: sites, perfumes, prod, Write one scan's reading of one size, and return its snapshot id.      The perfu (+23 more)
...
Cohesion: 0.11
Nodes (26): BasketReport, compare_split_to_best_full(), Every site's single-site scenario, split by whether it covers everything.      A, The cheapest basket split the search found. A heuristic, not a proof.      Every, Score a split plan against the cheapest full-coverage single site.      Only the, SplitPlan, _heading(), _leg_block() (+18 more)
...
- **Why does `SearchScreen` connect `TUI App Shell` to `TUI App & Screens`, `Site Profiles & Templates`, `Title Matcher`, `Search/Basket Domain Models`, `Search Engine per Site`, `ScanStatus.tsx`, `Search TUI Screen`, `conftest.py`, `Price/Size Normalization`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `fetch()` connect `HTTP/Browser Fetching` to `Search Engine Core`, `Fixture Fetcher (Tests)`, `JsonLdProduct`, `._build_rows`, `Playwright Errors`, `Basket Site Scenarios`, `_named_profile`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `BasketScreen` connect `Price/Size Normalization` to `TUI App & Screens`, `Site Profiles & Templates`, `Title Matcher`, `TUI App Shell`, `Headers`, `Basket Optimizer Core`, `ScanStatus.tsx`, `conftest.py`?**
